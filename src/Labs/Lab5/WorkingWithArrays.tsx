...
export default function WorkingWithArrays() { 
  ...
  return ( 
    ...
    <a id="wd-retrieve-todos" className="btn btn-primary" href={API}> 
      ...
      <br /><br /><hr />
      <h3>Update Todo Compelete Status</h3> 
      <a id="wd-update-completed-todos" className="btn btn-primary float-end" 
          href={`${API}/${todo.id}/completed/${todo.completed}`}> 
        Complete Todo ID = {todo.id} 
      </a> 
      <input value={todo.id} className="form-control w-25 float-start me-5" 
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <input value = {todo.id} type="checkbox" className="form-check-input" id="wd-todo-completed"
